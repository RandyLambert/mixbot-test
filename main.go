...
func main() {
	...
	fmt.Println(viper.GetString("mixin.UserID"),
		viper.GetString("mixin.SessionId"),
		viper.GetString("mixin.PrivateKey"),
		viper.GetString("mixin.PinToken"),
		viper.GetString("mixin.PinCode"),"/n")
	...
	doWithdraw(ctx, user, assetID, publicKey1, "0.01", "ping", viper.GetString("mixin.PinCode"))// ??u转账给user
	time.Sleep(time.Second * 5)
